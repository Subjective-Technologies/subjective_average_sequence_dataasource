...
class SubjectiveAverageSequenceDataSource(SubjectiveDataSource):
    connection_type = "InternalAnalytics"
    connection_fields = ["symbol", "window_size", "interval"]
    ...
    def fetch(self):
        ...
        name = self._get_param("name", "average_sequence_per_symbol_in_the_last_minutes")
        ds = AverageSequenceDataSource(name=name)
        data = ds.average_sequence_per_symbol()
